%%%%%%%%%%%%%%%%%%%%%    Full MD program for 2D (Velocity Verlet with Berendsen thermostat + LJ interactions)  %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
T(1)=Tref;K(n)=0.5*sumkin;U(n)=0;E(n)=K(n)+U(n);
%%
...
K(n)=0.5*sumkin;E(n)=K(n)+U(n);T(2)=tempa;
%%      
 %%%%%%%%%%%%%%%%%%%%%% LOOP OVER TIME %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
     tscale=0.5;  
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
   ...
   for k=1:npart       % update accelerations
    ...
    if n>300
    vx(n,k)=((1+(dt/tscale)*((Tref/temp(n,k))-1)))^(1/2)*vx(n,k);  % Berendsen thermostat
    vy(n,k)=((1+(dt/tscale)*((Tref/temp(n,k))-1))^(1/2))*vy(n,k);  % Berendsen thermostat
    end
